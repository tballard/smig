 ...
 * This was my first project, so if it's lame, feel free to give me a clue.
...
/** Private base class for unit values */
...
class AL private (f: Float) extends UV { 
...
class LPX private (f: Float) extends UV {
...
/** Specialize UV for logical pixels so spaces try to grow with fonts. */
class LPY private (f: Float) extends UV {
...
class PCT private (percent: Float) extends UV { 
...
class PX private (f: Float) extends UV {
...
class SPX private (f: Float) extends UV {
...
/** Specialize UV for, you know, Screen Percentatge */
class SPY private (f: Float) extends UV {
...
class IN private (f: Float) extends UV {
...
class MM private (f: Float) extends UV {
...
class CM private (f: Float) extends UV {
...
/** FS means freestyle.  Allow inputting any old crap without any checking.
 X means horizontal */
class FSX private (s: String) extends UV {
  _value = ConstraintParser.parseUnitValue(s, true)
}
object FSX {
  def apply(s: String) = new FSX(s)
}
  
/** FS means freestyle.  Allow inputting any old crap without any checking.
 Y means vertical */
class FSY private (s: String) extends UV {
  _value = ConstraintParser.parseUnitValue(s, false)
}
object FSY {
  def apply(s: String) = new FSY(s)
}
...
class LC private[smig] (lc: net.miginfocom.layout.LC) {
  ...
  private def this() = this(new net.miginfocom.layout.LC())
    
  /** The number of column/row constraints specified will be used. */
  def wrap : this.type = { _lc.wrap; this }
  ...
  def getAlignX : Option[AlignX] = {
    ...
    if (unitValue == null) None else Some(AlignX.toAlignX(unitValue.getValue))
  }
        
  def getAlignY : Option[AlignY] = {
    val unitValue = _lc.getAlignY
    if (unitValue == null) None else Some(AlignY.toAlignY(unitValue.getValue))
  }
   ...
   * the first/last column/row overrides these layout insets. This is the same
   * thing as setting an EmptyBorder on the container but without removing any
   * border already there. Default value is "panel" (or zero if there are
   * docking components). The size of "dialog" and "panel" insets is returned
   * by the current PlatformConverter. The inset values all around can also be
   * set explicitly for one or more sides. Insets on sides that are set to
   * N will get the default which is the platform default for "panel".
   */
   ...
   * only be needed if the "%" unit is used as it is a function of the parent
...
abstract class AC protected[smig] (ac: net.miginfocom.layout.AC) { 
   ...
   * constraints). Note that this property does not affect the size for the row,
  ...
  /** Set the index of the nect column to specify properties of, starts at 0 */
...
extends Panel with LayoutContainer {
  override lazy val peer =
    new JPanel(new MigLayout(
        lc match { case None => null; case Some(lc) => lc.java },
  ...
  private def str(bs: BoundSize) : String = {
    if (bs == null) null else {
      val s = str(bs.getMin) + ":" + str(bs.getPreferred) + ":" + str(bs.getMax)
  ...
  def getLC : Option[LC] = {
    val lc = 
      layoutManager.getLayoutConstraints.asInstanceOf[net.miginfocom.layout.LC]
    if (lc == null) None else Some(new LC(lc))
  }
  ...
  class Constraints private[smig] (cc: net.miginfocom.layout.CC) {
    ...
    def minWidth(bs: BS) : this.type = { _cc.minWidth(bs.toString); this }
    
    /** The size for the component as a bound size */
    def width(bs: BS) : this.type = { _cc.minWidth(bs.toString); this }
    
    /** The maximum size for the component. The value will override any value 
     that is set on the component itself. */
    def maxWidth(bs: BS) : this.type = { _cc.maxWidth(bs.toString); this }
    ...
    /** Skip this many cells before placing.  If not given, skip is 1 in 
     * flow direction */
    def skip(i: Int) : this.type = { _cc.skip(i); this }
    
    /**
     * The number of cells the cell that this constraint's component
     * will span in the indicated dimension. 1 is default and means that it
     * only spans the current cell.  With no argument spans all.
     */
